...
// Types for testimonial feedback
...
export default function ProfilePage() {
  ...
  // Calculate average rating based on testimonials.
  const calculateAverageRating = (testimonials: Testimonial[] | undefined) => {
    if (!testimonials || testimonials.length === 0) return 0;
    const total = testimonials.reduce((sum, t) => sum + t.rating, 0);
    return Math.round((total / testimonials.length) * 10) / 10;
  };

  // Return top testimonials (sort descending by rating).
  const getTopTestimonials = (testimonials: Testimonial[] | undefined) => {
          ...
          <Card className="shadow-md">
            <CardContent className="pt-6">
              <div className="flex flex-col items-center">
                ...
                {/* Feedback & Testimonials Section (only for freelancers) */}
                {profile?.role === "freelancer" && (
                  <>
                    <Separator className="my-4" />
                    <div className="w-full text-center">
                      <h3 className="text-lg font-bold mb-2">Feedback & Testimonials</h3>
                      <p className="text-sm text-muted-foreground">
                        Average Rating: {calculateAverageRating(profile.testimonials)} / 5
                      </p>
                    </div>
                    {profile.testimonials && profile.testimonials.length > 0 ? (
                      <div className="mt-3 space-y-3">
                        {getTopTestimonials(profile.testimonials).map((testimonial, index) => (
                            ...
                            <div className="flex items-center gap-3 mb-2">
                              ...
                              <div>
                                <p className="font-semibold text-sm">
                                  {testimonial.client?.name || "Unknown"}
                                </p>
                      ...
                      <p className="text-sm text-muted-foreground mt-2">No testimonials yet.</p>
                    )}
                  </>
                )}
              </div>
