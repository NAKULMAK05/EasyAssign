...
export default function MyTasksPage() {
  ...
  // When the user marks a task complete, update its status then show the feedback dialog.
  const handleMarkComplete = async (taskId: string) => {
    try {
      ...
      // Set the current task id for which feedback is being given.
  ...
  // Submit feedback for the freelancer and redirect to tasks page.
  const handleSubmitFeedback = async () => {
    try {
      const token = localStorage.getItem("token");
      // Find task to obtain freelancer id.
      const task = tasks.find(t => t._id === completeTaskId);
      if (!task) return;
      // Assuming task.applications[0].applicant is the freelancer id (update as needed)
      ...
      // Clear feedback fields, close dialog, and redirect to tasks page.
      setFeedbackRating(5);
      setFeedbackComment("");
...
function TaskCard({ task, onDelete, onMarkComplete }: { task: Task; onDelete: () => void; onMarkComplete: () => void; }) {
  return (
