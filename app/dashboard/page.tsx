...
export default function DashboardPage() {
  ...
  // Use an array for selectedFiles to allow removing files
  ...
  const avgBudget = totalTasks > 0 ? Math.round(tasks.reduce((acc, t) => acc + Number(t.budget), 0) / totalTasks) : 0
  const totalCategories = new Set(tasks.map((t) => t.category)).size

  // Change the type for selected files from FileList -> File[] and update onChange handler.
  ...
  // Function to remove a file from selectedFiles list
  ...
  // TaskCard uses postedBy fields (as in your task details page) for proper client name and avatar
  function TaskCard({ task }: { task: Task }) {
    ...
    // Always use postedBy for client details
